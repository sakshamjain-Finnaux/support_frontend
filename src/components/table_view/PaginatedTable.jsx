...
export default function PaginatedTable({ data, columns, searchText, queryPageIndex, queryPageSize, dispatch, hasmore }) {
                ...
                <table className="w-full sm:shadow-sm" {...getTableProps()}>
                    ...
                    <tbody {...getTableBodyProps()} className="flex-1 sm:flex-none">
                        {page.map((row) => {
                            prepareRow(row);
                            return (
                                <tr {...row.getRowProps()} className={`text-center duration-150 hover:bg-body-940 even:bg-body-800 ${row.original.variant == 'red' ? "!bg-red-900/20" : ""}`}>
                                    {row.cells.map((cell) => {
                                        return (
                                            <td
                                                {...cell.getCellProps()}
