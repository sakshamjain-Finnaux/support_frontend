...
function Toast({ message, type, id }) {
  ...
  return (
    <div
      ...
      className={
        "text-text flex items-center bg-body-900 shadow-md rounded-md p-4 relative w-96 overflow-hidden border-2 border-body-800 animate-[slide-in_0.4s_ease-in-out_forwards] " +
        (dismissed
          ? "animate-[slide-out_0.4s_ease-in-out_forwards] z-[51]"
          : "")
      }
    >
      ...
      <button className="cursor-pointer border-none bg-none ml-auto">
        <CloseIcon
          className="text-md text-text hover:text-text-dark"
      ...
      <div className="absolute bottom-0 left-0 w-full h-1 bg-black bg-opacity-10">
        <div
          ref={progressRef}
          className={`h-full animate-[progress-bar_4s_linear_forwards] ${progressBarClass}`}
        ></div>
      </div>
    </div>
  );
