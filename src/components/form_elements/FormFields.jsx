...
export function InputField({ register, validationSchema, placeholder, title, errors, name, type = "text", disabled = false }) {
    return (
        <div className="w-full relative">
            ...
            {errors?.[name] && <span className="absolute -bottom-4 text-xs text-error">{errors?.[name].message}</span>}
        </div>
    )
}

export function PasswordField({ disabled = false, register, validationSchema, placeholder, title, errors, name }) {
    ...
    return (
        <div className="w-full relative">
            <label className="">
                <div className='mb-1'>{title}</div>
                <div className={`shadow-inner flex border-2 rounded-lg bg-body-940 ${errors[name] ? "border-red-700 hover:border-red-900 ring-red-800" : "border-body-700 hover:border-body-800 ring-primary-700"}`}>
                    <input
                        disabled={disabled}
...
export function TextAreaField({ disabled = false, register, validationSchema, placeholder, title, errors, name }) {
    return (
        <div className="w-full relative">
            <label className="block">
                <div className="mb-1"> {title}</div>
...
export function BrowseFileField({ disabled = false, register, validationSchema, title, errors, name }) {
    return (
        <div className="w-full relative">
            <label>
                <div className='mb-1'>{title}</div>
