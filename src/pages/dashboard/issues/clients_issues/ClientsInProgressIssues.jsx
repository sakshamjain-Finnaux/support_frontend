...
export default function ClientsInProgressIssues() {
    ...
    async function fetchData(pageIndex, pageSize) {
        const result = await networkRequest("clients_inprogress_issues", { pageIndex, pageSize });
        if (!result.success) throw result;
        return result.data;
    }

    const query = useQuery({
        queryKey: ["issues", "clients", "inprogress", queryPageIndex, queryPageSize],
    ...
    const data = useMemo(() => {
        ...
        const data = query.data?.rows.map(row => {
            ...
            row["action"] = (
                <ActionMenu>
                    <button
                        onClick={() => showInOverlay(<IssueTimeLineView networkRequest={networkRequest} issueId={row.issue_id} />, false)}
                    >
                        View
                    </button>
                </ActionMenu>
