    ...
    return (
        <div className="w-[40rem] max-w-full bg-body-940 rounded-lg">
            <Tab.Group>
                ...
                <Tab.List as="div" className="top-0 left-0 z-[1] bg-inherit flex w-full border-body-700 border-b-2 rounded-t-lg overflow-hidden sticky">
                    {tabs.map((tab) => (
                        <Tab
                            key={tab}
                            className={({ selected }) => (
                                `w-full py-2.5 text-sm font-medium leading-5 ${selected
                ...
                <Tab.Panels className="text-sm">
                    ...
                    <Tab.Panel>
                        <IssueTimeLineView resolveButton={assignFieldsQuery.data.resolve} showRemark={true} networkRequest={networkRequest} issueId={issueId} />
                    </Tab.Panel>

                    <Tab.Panel>
                        <AssignForm show={assignFieldsQuery.data} networkRequest={networkRequest} issueId={issueId} />
                    </Tab.Panel>
