...
import { usePostEditEmployeeData, useGetEditEmployeeData } from "../../../hooks/useEditEmployee";
...
export default function EditEmployee() {
    ...
    const { showConfirmDialog, setShowFullSpinner, showInOverlay, toast } = useUI();
    ...
    if (query.isError) {
        return (
            <div className='flex flex-grow items-center justify-center flex-col'>
                <p className='font-medium text-2xl text-body-500'>Invalid Employee Id!</p>
            </div>
        )
    }
    ...
    return (
        <div className="flex-grow gap-6 flex flex-col">
            ...
            <EmployeeForm
                ...
                resetPassword={() => showConfirmDialog("Reset Password!", resetPasswordMutation.mutate)}
                inactiveCallback={() => showConfirmDialog("Make Employee Inactive!", inactiveMutation.mutate)}
                type="edit"
            />
        </div >
    )
...
function ShowNewPassword({ formData }) {
    ...
    const { register, formState: { errors } } = formData ? useForm({
        defaultValues: {
            ...formData
        }
    }) : useForm();
