...
export function AuthProvider({ children }) {
  ...
  const signin = async (data) => {
    ...
    result.data.permissions = Object.keys(result.data.permissions).filter(
      (permission) => result.data.permissions[permission],
    );
    setUser(result.data);
  ...
  const value = useMemo(
    () => ({ user, signin, signout, invalidateUser, hasPermission }),
    [user],
  );
