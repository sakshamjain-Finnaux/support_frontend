...
export function useGetEditEmployeeData(networkRequest, emp_id) {
  async function fetchData() {
    ...
    if (result.data.subordinates) {
      ...
      result.data.subordinates = result.data.subordinates.map(
        (subordinate) => subordinate.emp_id,
      );
    }
    if (result.data.media?.length) {
      result.data.media[0] = await utilities.getFileFromUrl(
        url + "/" + result.data.media[0],
        "profile",
      );
    }
    return result.data;
